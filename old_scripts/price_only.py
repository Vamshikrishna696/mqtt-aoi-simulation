...
for t in range(T):
    ...
    chosen = [n for n in order if scores[n] > lam]
...
print("MODE: price-only (no hard cap)")
...
plt.ylabel("lambda")
plt.title("Lambda (price-only)")
plt.tight_layout()
plt.savefig("lambda_price_only.png", dpi=300)
plt.show()
...
plt.title("Tx per slot (price-only)")
plt.tight_layout()
plt.savefig("tx_price_only.png", dpi=300)
plt.show()
...
plt.title("Average AoI (price-only)")
plt.tight_layout()
plt.savefig("avg_aoi_price_only.png", dpi=300)
plt.show()